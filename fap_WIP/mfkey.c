...
void quicksort(unsigned int array[], int low, int high) {
    while (low < high) {
        ...
        if (j - low < high - i) {
            if (low < j) quicksort(array, low, j);  // Sort left partition
            low = i;  // Continue with right partition
        } else {
            if (i < high) quicksort(array, i, high);  // Sort right partition
            high = j;  // Continue with left partition
        }
    }
}
